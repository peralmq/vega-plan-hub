// The recipe markdown parser, extracted from src/services/recipeLoader.ts by
// p4-03 (Decision Log: "shared loader, not a build-time mirror"). Everything
// here is pure string work with no Vite-only constructs, so BOTH surfaces run
// the same parser: the web app bundles the corpus with `import.meta.glob`
// (recipeLoader.ts, which re-exports this module) and the bot reads the same
// files from its checkout with `fs` (bot/recipeLibrary.ts). One parser, one
// format contract (docs/specs/recipe-format.spec.md) — a mirror would have to
// be regenerated and could go stale between a p4-08/09 recipe commit and the
// next build.
